...
class HomeWorkTest(TestCase):
        def setUp(self):
                ...
                self.assertEqual(response.status_code, 200, msg="Ошибка, страница пользователя не найдена!")
        ...
        def test_post_add_edit(self):                
                ...
                self.assertContains(response, new_text, count=None, status_code=200, msg_prefix='', html=False)
                ...
                response = self.client.get("/")
                self.assertContains(response, update_text, count=None, status_code=200, msg_prefix='', html=False)
        ...
        def test_check_imd(self):
                ...
                new = Post.objects.create(author=self.user, text=new_text, image = image_file)
                post_id = new.id
                ...
                with open(noimage_file, mode='rb') as fi:
                    edit_post = self.client.post(f"/{self.user.username}/{post_id}/edit/", {'text': new_text, 'image': fi})
        ...
        def test_check_change(self):    
                ...
                self.assertNotContains(new_post, change_text, status_code=302, msg_prefix='', html=False)

                #ждем 20 секунд и проверяем, текст должен появиться, иначе что то пошло не так
                time.sleep(21) 

                response = self.client.get("/")
                self.assertContains(response, change_text, count=None, status_code=200, msg_prefix='', html=False)
        ...
        def test_check_follow(self):    
                ...
                self.assertContains(response, "Отписаться", count=None, status_code=200, msg_prefix='', html=False) 
                response = self.client.get(f"/follow/")
                self.assertContains(response, check_text, count=None, status_code=200, msg_prefix='', html=False) 

                unfollow_user = self.client.get(f"/{page_user}/unfollow")
                response = self.client.get(f"/{page_user}/")
                self.assertContains(response, "Подписаться", count=None, status_code=200, msg_prefix='', html=False) 
                response = self.client.get(f"/follow/")
                self.assertNotContains(response, check_text, status_code=200, msg_prefix='', html=False) 

                response = self.client.get(f"/{page_user}/{post_id}/")
                self.assertContains(response, "Добавить комментарий", count=None, status_code=200, msg_prefix='', html=False)

                self.client.logout()
                response = self.client.get(f"/{page_user}/{post_id}/")
                self.assertContains(response, "Что бы добавить комментарий, необходимо авторизироваться", count=None, status_code=200, msg_prefix='', html=False)               